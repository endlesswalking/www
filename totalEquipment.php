...
<center>
    <div id="container">
        ...
        <div id="content">
            ...
            <div><br>
            <br /><br /><br />
            <form action="totalEquipment.php" method="post">
            請點選類別:
            <input name="classs" type="radio" value="C0001">書
            <input name="classs" type="radio" value="C0002">電腦
            <input name="classs" type="radio" value="C0003">會議室
            <input name="classs" type="radio" value="C0004">電器
            <input name="classs" type="radio" value="C0005">音效設備
            <input name="classs" type="radio" value="C0006">日常用品
            ...
            <?php	
				if(isset($_POST["classs"]))
				{
					...
					@ $query = mysql_query("SELECT eId,eName
											FROM equipment
											WHERE equipment.cId = '$classs' AND (eId,eName)NOT IN(SELECT equipment.eId,equipment.eName
				                                                                  FROM rent JOIN equipment ON rent.eId = equipment.eId JOIN class ON                                              equipment.cId = class.cId 
										                                          WHERE class.cId='$classs' AND rent.rStime <= '$nowTime' AND rent.rEtime                                              > '$nowTime') ");
					$row = mysql_num_rows($query);
					$field = mysql_num_fields($query);
					$i=0; $j=0;
					?>
					<table border="1" height="120" cellspacing="0">
                    <caption>未借出的物品</caption>
					<tr><th width="100" height="50">編號
           				<th width="600" height="50">名稱</th>
					<?php
					for($i=0;$i<$row;$i++)
					{?><TR><?php
						for($j=0;$j<$field;$j++)
						{?><td align="center" height="32"><?php
					...
					?><br><br>
					<?php
					@ $query = mysql_query("SELECT equipment.eId,equipment.eName,rStime,rEtime 
											FROM rent JOIN equipment ON rent.eId = equipment.eId JOIN class ON                                              equipment.cId = class.cId 
											WHERE class.cId='$classs' AND rent.rStime <= '$nowTime' AND rent.rEtime                                              > '$nowTime' ");
					...
					else
					{
						...
						<caption>借出的物品</caption>
						<tr><th width="100" height="50">編號
							<th width="600" height="50">名稱</th><th width="100">開始時間</th>
							<th width="100">結束時間</th></tr><?php
						for($j=0;$j<$row;$j++)
						{
							?><tr><?php
							for($i=0;$i<$field;$i++)
							{
								$result = mysql_result($query,$j,$i);
								?><td align="center"><?php
								echo $result;
								?></td>
								<?php
