        ...
        <div id="content">
            ...
            <?php
				...
				if ((isset($_SESSION["nowId"])))
				{
					...
					@ $query = mysql_query("SELECT class.cName, equipment.eName, rent.rStime, rent.rEtime 
FROM equipment JOIN rent ON equipment.eId = rent.eId JOIN class ON equipment.cId = class.cId 
WHERE rent.sId = '$id'");
					...
					<tr><th width="70" height="60">類別</th>
                    	<th width="400">物品名稱</th>
                    	<th width="130">借出時間</th>
                        <th width="130">歸還時間</th></tr>
					<?php
					for($i=0;$i<$row;$i++)
					{?><TR><?php
						for($j=0;$j<$field;$j++)
						{?><td align="center" height="40"><?php
