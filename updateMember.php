    ...
    <div id="container">
        ...
        <div id="content">
           ...
           <form action="updateMember.php" method="post">
            ...
            信箱:<input name="email" type="text"><br>
            ...
            <?php	
				...
				if(isset($_POST["sId"]))
				{
					...
					$i = 0;
					$j = 1;
					$password = mysql_result($data,$i,$j);
					$j++;
					$name = mysql_result($data,$i,$j);
					$j++;
					$phone =  mysql_result($data,$i,$j);
					$j++;
					$email =  mysql_result($data,$i,$j);
					...
					if((isset($_POST["sId"])))
					{
						@ $query = mysql_query("UPDATE `group11`.`student` SET `sPassword` = '$password', `sName` = '$name', `sPhone` = '$phone', `sEmail` = '$email' WHERE `student`.`sId` = '$sId'");
